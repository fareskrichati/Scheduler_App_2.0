...
exports.handler = async function handler(event) {
  if (event.httpMethod === "GET") {
    return json(200, { configured: Boolean(process.env.OPENAI_API_KEY && process.env.SUPABASE_URL && process.env.SUPABASE_ANON_KEY) });
  }
...
async function requirePlannerUser(authorization) {
  ...
  if (!url || !anonKey) throw Object.assign(new Error("Backend authentication is not configured."), { statusCode: 503 });
